...
import { supportstWorkerType } from "../../../lib/shared/util.js"

async function getWorker(){
  if(supportstWorkerType()) {
    console.log('Initializing esm workers')
    const worker = new Worker('./src/worker.js', { type: 'module'})
    return worker
  } 
  console.warn(`Your browser doesn't supoort esm modules on webworkers!`)
  console.warn(`Import librares...`)
  ...
  const workerMock = {
    async postMessage(video) {
      const blinked = await service.handBlikend(video)
      if(!blinked) return;
